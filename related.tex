...
Finally, our correctness results are slightly stronger, as they assume well-typed programs that cannot go wrong, while we work on an untyped calculus and prove a behavioral refinement of failures: the target can get stuck only when the source can.
We get this extra generality mostly for free by defining the simulation carefully, but it requires some care when handling type failures arithmetic operators in the accumulator-passing-style variant.
