...
\Xgabriel{TODO Clément re-read this.} In simple terms, the admissibility condition states that the protocol can be realized by a concrete function that takes a pair of expressions related in the protocol with a postcondition $\iPredTwo$, and produces related reduction sequences (one simultaneous step, after potential stuttering on both sides) that end up in the desired postcondition $\iPredTwo$.
