...
\paragraph{Comparing \bench{tmc} and \bench{batteries}} It is interesting to compare \bench{tmc} and \bench{batteries} because they are both TMC versions, with a different design choice. The Batteries implementation, as Koka~\citep*{tmc-koka-2023}, only generates the DPS version; the direct-style function is a short wrapper that allocates an extra dummy block to serve a destination, and calls the DPS version. This strategy reduces code size, at the cost of an extra allocation in all cases. The \OCaml implementation instead generates a direct-style version that is similar to the input function, effectively doubling the code size, and only calls the DPS version under block constructors. This choice guarantees that the TMC version does not perform any extra allocation, which some \OCaml users want to be able to reason about. The benchmarks show that this does not affect the performance in practice, in a micro-benchmark scenario.
